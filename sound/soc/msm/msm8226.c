...
#ifdef CONFIG_KYOCERA_MSND
#define LO_1_RCV_AMP   0x1
#define LO_2_RCV_AMP   0x2
...
#ifdef CONFIG_KYOCERA_MSND
static int msm8226_ext_rcv_pamp;
#else
...
#ifdef CONFIG_KYOCERA_MSND
static int ext_rcv_amp_gpio_on = -1;
static int ext_rcv_amp_gpio_gain = -1;
static int ext_rcv_amp_gpio_sel = -1;
#else
...
#ifdef CONFIG_KYOCERA_MSND
static void msm8226_ext_rcv_power_amp_enable(u32 enable)
{
	if ( !kc_setup_gain ) {
		kc_read_gain_data();
		kc_setup_gain = true;
	...
	if (enable) {
		if (kc_rcv_sel) {
			gpio_direction_output(ext_rcv_amp_gpio_sel, 1);
		} else {
			gpio_direction_output(ext_rcv_amp_gpio_sel, 0);
		}
		if (kc_rcv_gain) {
			gpio_direction_output(ext_rcv_amp_gpio_gain, 1);
		} else {
			gpio_direction_output(ext_rcv_amp_gpio_gain, 0);
		}
		gpio_direction_output(ext_rcv_amp_gpio_on, 1);
		usleep_range(15000, 15000);
	} else {
		gpio_direction_output(ext_rcv_amp_gpio_on, 0);
		gpio_direction_output(ext_rcv_amp_gpio_sel, 0);
		gpio_direction_output(ext_rcv_amp_gpio_gain, 0);
	}
...
static void msm8226_ext_rcv_power_amp_on(u32 rcv)
{
	if ((gpio_is_valid(ext_rcv_amp_gpio_on)) &&
		(gpio_is_valid(ext_rcv_amp_gpio_gain)) &&
		(gpio_is_valid(ext_rcv_amp_gpio_sel))) {
		if (rcv & (LO_1_RCV_AMP | LO_2_RCV_AMP)) {
			pr_debug("%s:Enable left and right receiver case spk = 0x%x\n",
				__func__, rcv);

			msm8226_ext_rcv_pamp |= rcv;

			if ((msm8226_ext_rcv_pamp & LO_1_RCV_AMP) &&
				(msm8226_ext_rcv_pamp & LO_2_RCV_AMP)) {
				pr_debug("%s  enable power", __func__);
				msm8226_ext_rcv_power_amp_enable(1);
			}
		} else  {
...
static void msm8226_ext_rcv_power_amp_off(u32 rcv)
{
	if ((gpio_is_valid(ext_rcv_amp_gpio_on)) &&
		(gpio_is_valid(ext_rcv_amp_gpio_gain)) &&
		(gpio_is_valid(ext_rcv_amp_gpio_sel))) {
		if (rcv & (LO_1_RCV_AMP | LO_2_RCV_AMP)) {
			...
			if (!msm8226_ext_rcv_pamp) {
				pr_debug("%s  disable power", __func__);
				msm8226_ext_rcv_power_amp_enable(0);
				msm8226_ext_rcv_pamp = 0;
			}
		 } else  {
...
static int msm8226_ext_rcvamp_event(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *k, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		if (!strncmp(w->name, "Lineout_1 amp", 14))
			msm8226_ext_rcv_power_amp_on(LO_1_RCV_AMP);
		else if (!strncmp(w->name, "Lineout_2 amp", 14))
			msm8226_ext_rcv_power_amp_on(LO_2_RCV_AMP);
		else {
		...
		if (!strncmp(w->name, "Lineout_1 amp", 14))
			msm8226_ext_rcv_power_amp_off(LO_1_RCV_AMP);
		else if (!strncmp(w->name, "Lineout_2 amp", 14))
			msm8226_ext_rcv_power_amp_off(LO_2_RCV_AMP);
		else {
			pr_err("%s() Invalid Receiver Widget = %s\n",
				__func__, w->name);
			return -EINVAL;
...
#ifdef CONFIG_KYOCERA_MSND
	SND_SOC_DAPM_SPK("Lineout_1 amp", msm8226_ext_rcvamp_event),
	SND_SOC_DAPM_SPK("Lineout_2 amp", msm8226_ext_rcvamp_event),
...
static __devinit int msm8226_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (OEM_get_board() != OEM_BOARD_WS0_TYPE) {
		ext_rcv_amp_gpio_on = of_get_named_gpio(pdev->dev.of_node,
					"kc,rcv-amp-gpio-on", 0);
		if (ext_rcv_amp_gpio_on < 0) {
			dev_err(&pdev->dev, "Looking up %s propertyd %d\n", pdev->dev.of_node->full_name, ext_rcv_amp_gpio_on);
		} else {
			ret = gpio_request(ext_rcv_amp_gpio_on, "RECEIVER_AMP_ON");
			if (ret) {
				dev_err(card->dev,
						"%s: Failed to request rcvr gpio on %d\n", __func__, ext_rcv_amp_gpio_on);
				goto err_vdd_spkr;
			}
		}

		ext_rcv_amp_gpio_sel = of_get_named_gpio(pdev->dev.of_node,
					"kc,rcv-amp-gpio-sel", 0);
		if (ext_rcv_amp_gpio_on < 0) {
			dev_err(&pdev->dev, "Looking up %s property %d\n", pdev->dev.of_node->full_name, ext_rcv_amp_gpio_sel);
		} else {
			ret = gpio_request(ext_rcv_amp_gpio_sel, "RECEIVER_AMP_SEL");
			if (ret) {
				dev_err(card->dev,
						"%s: Failed to request rcvr gpio sel %d\n", __func__, ext_rcv_amp_gpio_sel);
				goto err_lineout_spkr;
			}
		}

		ext_rcv_amp_gpio_gain = of_get_named_gpio(pdev->dev.of_node,
					"kc,rcv-amp-gpio-gain", 0);
		if (ext_rcv_amp_gpio_gain < 0) {
			dev_err(&pdev->dev, "Looking up %s property %d\n", pdev->dev.of_node->full_name, ext_rcv_amp_gpio_gain);
		} else {
			ret = gpio_request(ext_rcv_amp_gpio_gain, "RECEIVER_AMP_GAIN");
			if (ret) {
				dev_err(card->dev,
						"%s: Failed to request rcvr gpio gain %d\n", __func__, ext_rcv_amp_gpio_gain);
				goto err_lineout_spkr;
			}
		}
	}
...
#ifdef CONFIG_KYOCERA_MSND
	if (ext_rcv_amp_gpio_on >= 0) {
		gpio_free(ext_rcv_amp_gpio_on);
		ext_rcv_amp_gpio_on = -1;
	}
	if (ext_rcv_amp_gpio_sel >= 0) {
		gpio_free(ext_rcv_amp_gpio_sel);
		ext_rcv_amp_gpio_sel = -1;
	}
	if (ext_rcv_amp_gpio_gain >= 0) {
		gpio_free(ext_rcv_amp_gpio_gain);
		ext_rcv_amp_gpio_gain = -1;
	}
#else
	if (ext_spk_amp_gpio >= 0) {
