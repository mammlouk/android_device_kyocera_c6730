...
int kernel_disp_on_flg = 0;

void oem_kernel_init_disp_on( void )
{
#if 0
  mdss_fb_open(oem_save_fbi, 0);
  mdss_fb_pan_display(&(oem_save_fbi->var), oem_save_fbi);
  msleep(30);
#endif
}
				      ...
				      enum led_brightness value)
{
	...
	if(0 <= os_bl_lvl &&  os_bl_lvl < 10){
		bl_lvl = 24 * os_bl_lvl;
	}else if(10 <= os_bl_lvl &&  os_bl_lvl < 102){
		bl_lvl = (1044 * os_bl_lvl + 11640)/92;
	}else if(102 <= os_bl_lvl &&  os_bl_lvl <= 255){
		bl_lvl = (2489 * os_bl_lvl - 57426)/153;
	}
#endif
	if (!bl_lvl && value)
		bl_lvl = 1;
...
static int mdss_fb_alloc_fbmem_iommu(struct msm_fb_data_type *mfd, int dom)
{
	...
	if(oem_is_off_charge()){
		memset(virt, 0x00, 544 * 960 * 4);
	}else{
		/*kernel_disp_raw_bitmap_SR(0,0,544,960,startlogo,virt);*/
	}
