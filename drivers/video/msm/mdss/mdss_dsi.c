				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (!gpio_is_valid(ctrl_pdata->disp_det_gpio)) {
		...
		rc = gpio_request(ctrl_pdata->disp_det_gpio, "disp_det");
		if (rc) {
			pr_err("request DET gpio failed, rc=%d\n",
			       rc);
			gpio_free(ctrl_pdata->disp_det_gpio);
			return -ENODEV;
		}
		rc = gpio_tlmm_config(GPIO_CFG(
				ctrl_pdata->disp_det_gpio, 0,
				GPIO_CFG_INPUT,
				GPIO_CFG_PULL_DOWN,
				GPIO_CFG_2MA),
				GPIO_CFG_ENABLE);
		...
		if (rc) {
			pr_err("%s: unable to config tlmm = %d\n",
				__func__, ctrl_pdata->disp_det_gpio);
			gpio_free(ctrl_pdata->disp_det_gpio);
			return -ENODEV;
		}

		rc = gpio_direction_input(ctrl_pdata->disp_det_gpio);
		if (rc) {
			pr_err("set_direction for disp_en gpio failed, rc=%d\n",
			       rc);
			gpio_free(ctrl_pdata->disp_det_gpio);
