...
static bool pm8xxx_vib_chk_hapticstime(int value)
{
...
static int pm8xxx_vib_set_hapticstime(int value)
{
	if (value > HAPTICS_TIME_10 &&
...
static int pm8xxx_vib_haptics(int value)
{
	...
	if (pm8xxx_vib_chk_hapticstime(value))
	{
		/* haptics flg on */
		atomic_set(&haptics, VIB_HAPTICS_ON);
		/* add haptics time */
		value = pm8xxx_vib_set_hapticstime(value);
	}
