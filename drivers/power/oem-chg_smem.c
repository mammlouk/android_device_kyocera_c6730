...
/* set wireless charge status to shared memory */
void oem_chg_set_wchg_status(wchg_status status)
{
	if (smem_ptr == NULL) {
		pr_err("smem_ptr is NULL\n");
		return;
	}
	smem_ptr[SMEM_CHG_WCHG_STATUS] = (uint8_t)status;
	pr_info("wchg_st:%d\n", smem_ptr[SMEM_CHG_WCHG_STATUS]);
}
EXPORT_SYMBOL(oem_chg_set_wchg_status);
