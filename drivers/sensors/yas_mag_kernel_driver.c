...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct yas_matrix lab_static_matrix = {YAS_LAB_MAG_STATIC_MATRIX};


	struct yas_matrix ws1_2_static_matrix = {YAS_WS1_2_MAG_STATIC_MATRIX};
	...
	if (board_type == OEM_BOARD_WS1_2_TYPE) {
		data->distortion[0] = YAS_WS1_2_MAGCALIB_DISTORTION_1;
		data->distortion[1] = YAS_WS1_2_MAGCALIB_DISTORTION_2;
		data->distortion[2] = YAS_WS1_2_MAGCALIB_DISTORTION_3;
	}


	else if (board_type >= OEM_BOARD_LAB_TYPE) {
		data->distortion[0] = YAS_LAB_MAGCALIB_DISTORTION_1;
		data->distortion[1] = YAS_LAB_MAGCALIB_DISTORTION_2;
		data->distortion[2] = YAS_LAB_MAGCALIB_DISTORTION_3;
	}
	...
	if (board_type == OEM_BOARD_WS1_2_TYPE) {
		if (hwdriver.set_static_matrix != NULL) {
			rt = hwdriver.set_static_matrix(&ws1_2_static_matrix);
			if (rt < 0) {
		...
		if (hwdriver.set_static_matrix != NULL) {
			rt = hwdriver.set_static_matrix(&lab_static_matrix);
			if (rt < 0) {
				YLOGE(("hwdriver.set_static_matrix() failed[%d]\n", rt));
				goto err;
			}
